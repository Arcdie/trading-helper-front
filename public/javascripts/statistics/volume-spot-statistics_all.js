...
objects, moment, ChartCandles, IndicatorVolume, IndicatorSuperTrend, LightweightCharts
*/
...
const isLoadFromCacheFile = false;
...
$(document).ready(async () => {
  ...
  if (isLoadFromCacheFile) {
    const resultGetFile = await getCacheFile({
      fileName: 'volume-spot-statistics.json',
    });
    ...
    instrumentsDocs.forEach(doc => {
      const keyVolumeBounds = `INSTRUMENT:${doc.name}:VOLUME_BOUNDS`;
      const keyCandles = `INSTRUMENT:${doc.name}:CANDLES_${periodWithUpperCase}`;

      doc.original_data = resultGetFile[keyCandles] ?
        JSON.parse(resultGetFile[keyCandles]) : [];

      doc.instrument_volume_bounds = resultGetFile[keyVolumeBounds] ?
        JSON.parse(resultGetFile[keyVolumeBounds]) : [];
    });
  } else {
    ...
    // instrumentsDocs = resultGetInstruments.result.filter(d =>
    //   ['616f0f7190a7836ed8d5e203', '616f0f6c90a7836ed8d5e0a1', '616f0f7190a7836ed8d5e1ab'].includes(d._id),
    // );
    ...
    for await (const doc of instrumentsDocs) {
      ...
      if (!doc.is_futures) {
        ...
        doc.instrument_volume_bounds = resultGetInstrumentVolumeBounds.result || [];
      } else {
        doc.instrument_volume_bounds = [];
      }

      doc.original_data = await getCandlesData({
      ...
      cacheObj[keyCandles] = JSON.stringify(doc.original_data);
      cacheObj[keyVolumeBounds] = JSON.stringify(doc.instrument_volume_bounds);
