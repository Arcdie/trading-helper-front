...
$(document).ready(async () => {
  ...
  $instrumentsList
    .on('click', '.instrument', async function (elem) {
      ...
      const trades = await loadTrades({ instrumentName: instrumentDoc.name });
...
const loadTrades = async ({ instrumentName }) => {
  console.log('started loading');

  const resultGetFile = await makeRequest({
    method: 'GET',
    url: `/files/aggTrades/weekly/${instrumentName}/${instrumentName}.json`,
  });
  ...
  const fileData = resultGetFile;

  const splitByMinutes = [];
  let newSplit = [fileData[0]];

  let minute = new Date(parseInt(fileData[0][2], 10)).getUTCMinutes();

  for (let i = 1; i < fileData.length; i += 1) {
    const minuteOfTrade = new Date(parseInt(fileData[i][2], 10)).getUTCMinutes();
      ...
      splitByMinutes.push(
        newSplit.map(tradeData => {
          ...
          const originalTimeUnix = parseInt(
            (new Date(parseInt(timestamp, 10)).setSeconds(0)) / 1000, 10,
          );
