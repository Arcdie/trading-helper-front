...
let finishDatePoint = moment().startOf('hour');
...
$(document).ready(async () => {
  ...
  trading.$tradingForm.find('.action-block button')
    ...
    .on('click', function () {
      ...
      const instrumentDoc = instrumentsDocs.find(doc => doc._id === choosenInstrumentId);
      const firstCandle = instrumentDoc.candles_data[0];
...
const setHistoryMoment = async () => {
  ...
  // todo: InstrumentNew: set start candles period field

  // 1 August 2021
  finishDatePoint = moment({ day: 1, month: 7, year: 2021 });
  const finishDatePointUnix = moment(finishDatePoint).unix();

  instrumentDoc.candles_data.filter(({ time }) => getUnix(time) < finishDatePointUnix);
